...
from utils.handling import check_in_n_out_dates, hotel_price, _, hotel_address, hotel_rating
...
def get_hotels(msg: Message, parameters: dict) -> [list, None]:
    ...
    if not data or len(data['results']) < 1: return None
...
def request_hotels(parameters: dict, page: int = 1):
    ...
    try:
        ...
        if data.get('message'): raise requests.exceptions.RequestException
...
def choose_best_hotels(hotels: list[dict], distance: float, limit: int) -> list[dict]:
    ...
    if len(hotels) > limit: hotels = hotels[:limit]

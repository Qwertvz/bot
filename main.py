from utils.handling import internationalize as _, is_input_correct, get_parameters_information, make_message, steps, locales, logger_config, currencies, is_user_in_db, add_user, extract_search_parameters
...
BOT_TOKEN = "6279896773:AAH7GYpFDGrwwyymQ6IEtfMBBqRzWe7C578"
bot = telebot.TeleBot(BOT_TOKEN, parse_mode='HTML')


def get_locations(msg: Message) -> None:
    if not is_input_correct(msg): bot.send_message(msg.chat.id, make_message(msg, 'mistake_'))
    else:
        ...
        if not locations or len(locations) < 1: bot.send_message(msg.chat.id, str(msg.text) + _('locations_not_found', msg))
        elif locations.get('bad_request'): bot.send_message(msg.chat.id, _('bad_request', msg))
...
def keyboard_handler(call: CallbackQuery) -> None:
    ...
    if call.data.startswith('code'):
        if redis_db.hget(chat_id, 'state') != '1':
            ...
            else: redis_db.hincrby(chat_id, 'state', 3)
...
def get_search_parameters(msg: Message) -> None:
    ...
    if not is_input_correct(msg): bot.send_message(chat_id, make_message(msg, 'mistake_'))
...
def hotels_list(msg: Message) -> None:
    ...
    if not hotels or len(hotels) < 1: bot.send_message(chat_id, _('hotels_not_found', msg))
        ...
        for hotel in hotels: bot.send_message(chat_id, hotel)


@bot.message_handler(content_types=['text'])
def get_text_messages(message) -> None:
    if not is_user_in_db(message): add_user(message)

    state = redis_db.hget(message.chat.id, 'state')

    if state == '1': get_locations(message)
    elif state in ['2', '3', '4']: get_search_parameters(message)
    else: bot.send_message(message.chat.id, _('misunderstanding', message))


try: bot.polling(none_stop=True, interval=0)
except Exception as e: logger.opt(exception=True).error(f'Unexpected error: {e}')
